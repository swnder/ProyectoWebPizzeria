     ...
     <body class="bg-dark text-white">
          <?php
               if (isset($_GET['id'])){ //Solo para modificar
          		...
          		$conex = conexion();
     			$id = $_GET['id'];
     			$sql = "SELECT * FROM tamano WHERE id = '$id'";
                         ...
                         <form id="form_ciudad" onkeypress="if(event.keyCode == 13) event.returnValue =validarCampos();">
                              ...
                              <div class="form-group row mt-3">
                                   ...
                                   <div class="col-12 col-md-6">
                                        <label class="font-weight-bold" for="ciudad">TAMANO</label>
                                            <input type="text" class="form-control text-uppercase" name="tamano" id="tamano" placeholder="Ingrese Tamano" maxlength="50" value="<?php echo isset($reg['tamano']) ? $reg['tamano'] : '';?>" autofocus>
                                   </div>
                           					...
                           					<div class="form-group row">
                           						...
                           						<div class="col-12 col-md-4 mb-2">
                           							<button type="button" class="btn gris btn-block" onclick="window.location.href='tamano_lista.php';"><i class="fa fa-table"></i> Ir a lista de Tamano</button>
                           						</div>
                     					</div>
                                <input type="hidden" name="accion" id="accion">
                                <input type="hidden" name="id" id="id" value="<?php echo isset($_GET['id']) ? $_GET['id'] : '';?>">
                                <input type="hidden" name="tamano" id="tamano" value="<?php echo isset($reg['tamano']) ? $reg['tamano'] : '';?>">
               ...
               if (isset($_GET["accion"])){
                    if ($_GET["accion"] == "N"){
                         echo "<script>
                              ...
                              document.getElementById('titulo').innerHTML = 'ALTA DE TAMANO';
                         </script>";
                    }else if ($_GET["accion"] == "M"){
                         echo "<script>
                              document.getElementById('accion').value = 'M';
                              document.getElementById('titulo').innerHTML = 'MODIFICAR TAMANO';
          ...
          <script>
               ...
               function registrar(){
                    ...
                    $.ajax({
                         type: "POST",
                         dataType: 'html',
                         url: "../servicios/tamanoServicios.php",
                         data: "tamano=" + tamano  + "&accion=" + acc,
                         // data: datos,
                    }).done( function(resp){ //se ejecuta cuando la solicitud Ajax ha concluido satisfactoriamente
                         if (resp == 1){
                              alertify.warning("El Tamano  ya existe. Cambie por otro");
                              $("#tamano").focus();
                         }else if (resp == 2){
               ...
               function validarCampos(){
                     ...
                     if ($("#tamano").val().length < 5){
                         alertify.error("Ingrese como mínimo 5 caracteres la Marca");
                         $("#tamano").focus();
                    }else{
                         if ($("#accion").val() == "N"){
                              registrar();
                    ...
                    function(){
                         if($("#accion").val() == "N"){
                              ...
                              alertify.alert("Atención", "Operación cancelada",
                                   function(){
                                        window.location="tamano_lista.php";
                                   }
                              );
                         }
               ...
               function actualizar(){
                    ...
                    $.ajax({
                         ...
                         dataType: 'html',
                         url: "../servicios/tamanoServicios.php",
                         data: "tamano=" + tamano + "&accion=" + acc  + "&id=" + id,
                    }).done( function(resp){ //se ejecuta cuando la solicitud Ajax ha concluido satisfactoriamente
                         if (resp == 3){
                              ...
                              alertify.alert("Modificar", "Registro actualizado con éxito",
                                   function(){
                                        window.location="../forms/tamano_lista.php";
                                   }
                              );
                         }
                    }).fail( function(resp){ //se ejecuta en que caso de que haya ocurrido algún error
